...
export function FinalCTA() {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      if (wrapperRef.current) {
        tl.fromTo(
          wrapperRef.current,
          // Increased initial scale from 0.35 to 0.7 so it's larger when not fully scrolled
          { scale: 0.7, opacity: 0.4, y: 50 },
          { scale: 1, opacity: 1, y: 0, ease: "none", force3D: true }
        );
      }
    }, sectionRef);
  ...
  return (
    <section ref={sectionRef} className="relative h-[200vh] bg-[#0A0A0A]">
      <div 
        ref={stickyRef} 
        className="relative flex h-screen items-center justify-center overflow-hidden bg-[#0A0A0A]"
      >
        {/* Subtle background glow replacing the tunnel */}
        ...
        {/* OUTER WRAPPER: Handles GSAP Scale/Opacity Scroll Animation */}
        <div 
          ref={wrapperRef} 
          // Increased max-width to max-w-4xl to make the card significantly larger overall
          className="relative z-20 mx-4 w-full max-w-4xl md:mx-6" 
