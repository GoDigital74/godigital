...
// // // =========================================
// // // YOUTUBE VIDEO COMPONENT (Updated)
// // // =========================================
// // function YouTubeShowcase({ url }: { url: string }) {
...
// //   return (
// //     <motion.div 
// //       // Triggers the iframe to mount and play when 30% of the container is in the viewport
// //       onViewportEnter={() => setIsInView(true)}
// //       viewport={{ once: true, amount: 0.3 }}
// //       className="relative w-full aspect-video md:aspect-[21/9] bg-[#111] rounded-[2rem] overflow-hidden shadow-2xl border border-slate-200"
// //     >
// //       {isInView && (
// //         <iframe
// //           // The scaling hides the black bars when mapping 16:9 videos into a 21:9 container
// //           // pointer-events-none disables all clicking, pausing, and YouTube UI overlays
// //           className="absolute inset-0 w-full h-full scale-[1.05] md:scale-[1.35] pointer-events-none"
// //           src={`https://www.youtube.com/embed/${videoId}?autoplay=1&mute=1&controls=0&rel=0&modestbranding=1&playsinline=1&loop=1&playlist=${videoId}`}
// //           title="YouTube video player"
// //           frameBorder="0"
// //           allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
// //           allowFullScreen
...
// // =========================================
// // YOUTUBE VIDEO COMPONENT (Updated for Auto-Aspect Ratio)
// // =========================================
// function YouTubeShowcase({ url }: { url: string }) {
//   const [isInView, setIsInView] = useState(false);
...
//   // Dynamically apply aspect ratio and max-width based on video type
//   const containerClasses = isShort
//     ? "relative mx-auto w-full max-w-[400px] aspect-[9/16] bg-[#111] rounded-[2rem] overflow-hidden shadow-2xl border border-slate-200"
//     : "relative mx-auto w-full max-w-[1200px] aspect-video bg-[#111] rounded-[2rem] overflow-hidden shadow-2xl border border-slate-200";
...
//       {isInView && (
//         <iframe
//           // Removed the artificial scale classes. pointer-events-none keeps it unclickable.
...
// =========================================
// YOUTUBE VIDEO COMPONENT (Reduced Height)
// =========================================
function YouTubeShowcase({ url }: { url: string }) {
  const [isInView, setIsInView] = useState(false);
  ...
  if (!videoId) {
    return (
      <div className="w-full aspect-[21/9] bg-slate-100 rounded-[2rem] flex items-center justify-center border border-slate-200 text-slate-500 font-medium">
        Valid YouTube URL required.
      </div>
    );
  }

  // Reduced Height Ratios: 
  // Landscape videos now use aspect-[21/9] which is significantly shorter in height than aspect-video (16:9).
  // Shorts remain aspect-[9/16] so they don't distort, but get a max-height so they aren't overly tall.
  const containerClasses = isShort
    ? "relative w-full max-w-[350px] mx-auto aspect-[9/16] bg-[#111] rounded-[2rem] overflow-hidden shadow-2xl border border-slate-200"
    : "relative w-full aspect-[21/9] bg-[#111] rounded-[2rem] overflow-hidden shadow-2xl border border-slate-200";
  ...
  return (
    ...
    >
      {isInView && (
        <iframe
          // The scale-[1.35] is added back ONLY for standard landscape videos. 
          // This zooms the video in slightly to fill the shorter 21:9 frame and eliminates black bars.
          className={`absolute inset-0 w-full h-full pointer-events-none ${!isShort && "scale-[1.35]"}`}
          src={`https://www.youtube.com/embed/${videoId}?autoplay=1&mute=1&controls=0&rel=0&modestbranding=1&playsinline=1&loop=1&playlist=${videoId}`}
          title="YouTube video player"
          frameBorder="0"
          allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
  ...
  return (
    <>
      <main className="min-h-screen bg-white">
        ...
        {/* 5. FEATURED MEDIA (Images + Conditional Video) */}
          ...
          <section className="bg-slate-50 px-6 py-14 md:py-12">
            <div className="mx-auto max-w-7xl">
              ...
              <div className="grid gap-8 md:grid-cols-2 items-start">
                ...
                {/* Place video IN the grid ONLY if there is no 2nd image */}
                {project.videoUrl && (!project.images || project.images.length < 2) && (
                  <div className="flex flex-col gap-4 w-full h-full justify-center">
                    <YouTubeShowcase url={project.videoUrl} />
                    <div>
                      {/* <h4 className="font-bold text-[#0f172a]">Project Video</h4>
                      <p className="text-sm text-slate-500">Watch the final outcome</p> */}
                    </div>
                  </div>
                )}
              </div>
        ...
        {/* 6. YOUTUBE VIDEO SHOWCASE (Fallback if 2+ images exist) */}
        {project.videoUrl && project.images && project.images.length >= 2 && (
          <section className="bg-white py-12 md:py-16 px-4 md:px-8">
